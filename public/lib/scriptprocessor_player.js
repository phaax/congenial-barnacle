...
var setGlobalWebAudioCtx= function() {
	if (typeof window._gPlayerAudioCtx == 'undefined') {	// cannot be instantiated 2x (so make it global)
		...
		// Pre-register the AudioWorklet module so it is ready by the time
		// the first song loads (addModule is async but the file-load XHR gives
		// us enough time for it to complete before audio actually starts).
		window._gWorkletReady = false;
		if (window._gPlayerAudioCtx && window._gPlayerAudioCtx.audioWorklet) {
			window._gPlayerAudioCtx.audioWorklet
				.addModule('lib/adplug_worklet.js')
				.then(function()  { window._gWorkletReady = true; })
				.catch(function() { window._gWorkletReady = false; });
		}
	}
	try {			
...
// ******** internal utils (MUST NOT be ued outside of the player or respective backendAdapters --------------
		...
		*/
		...
		createScriptProcessor: function(audioCtx) {
			var channels = this._backendAdapter.getChannels();
			if (audioCtx.audioWorklet && window._gWorkletReady) {
				var workletNode = new AudioWorkletNode(audioCtx, 'adplug-processor', {
					numberOfInputs:     0,
					numberOfOutputs:    1,
					outputChannelCount: [channels]
				});
				workletNode.port.postMessage({ type: 'cfg', channels: channels });
				this._startWorkletProducer(workletNode, channels);
				return workletNode;
			}
			// Fallback: ScriptProcessorNode (deprecated but functional)
			var scriptNode = audioCtx.createScriptProcessor(SAMPLES_PER_BUFFER, 0, channels);
			scriptNode.onaudioprocess = fetchSamples;
			return scriptNode;
		},
		// Starts a setInterval loop that generates PCM chunks on the main thread
		// and transfers them to the AudioWorklet processor for playback.
		_startWorkletProducer: function(workletNode, channels) {
			var isStereo = channels > 1;
			var ch0 = new Float32Array(SAMPLES_PER_BUFFER);
			var ch1 = isStereo ? new Float32Array(SAMPLES_PER_BUFFER) : null;
			var fakeEvent = {
				outputBuffer: {
					numberOfChannels: channels,
					getChannelData: function(i) { return i === 0 ? ch0 : ch1; }
				}
			};
			// Interval ≈ chunk duration; pre-fill 2 chunks to prevent initial underrun.
			var chunkMs = Math.floor(SAMPLES_PER_BUFFER / (this._sampleRate || 44100) * 1000);
			function pushChunk() {
				window.player.genSamples(fakeEvent);
				var out0 = new Float32Array(ch0);
				var msg  = { type: 'chunk', ch0: out0 };
				var xfer = [out0.buffer];
				if (isStereo && ch1) {
					var out1 = new Float32Array(ch1);
					msg.ch1 = out1;
					xfer.push(out1.buffer);
				}
				workletNode.port.postMessage(msg, xfer);
			}
			pushChunk();
			pushChunk();
			this._workletInterval = setInterval(pushChunk, chunkMs);
		},
		createTickerScriptProcessor: function(audioCtx) {
			if (audioCtx.audioWorklet && window._gWorkletReady) {
				// setInterval is accurate enough for 60 fps VU-meter animation
				var stepMs = Math.floor(256 / (this._sampleRate || 44100) * 1000);
				this._tickerInterval = setInterval(calcTick, stepMs);
				// Return a silent node to satisfy the caller's .connect() call
				var dummy = audioCtx.createGain();
				dummy.gain.value = 0;
				return dummy;
			}
			// Fallback: ScriptProcessorNode (deprecated but functional)
			var scriptNode = audioCtx.createScriptProcessor(256, 0, 1);
